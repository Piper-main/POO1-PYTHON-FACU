# Los teléfonos de una empresa tienen el siguiente formato prefijo-número-
# extension donde el prefijo es el código del país +54, y la extensión tiene cuatro
# dígitos (por ejemplo +54-9115847-1056). Escribir un programa que pregunte por
# un número de teléfono con este formato y muestre por pantalla el número de
# teléfono sin el prefijo y la extensión.

print(" Ingrese su numero de telefono")
print(" Con el siguiente formato, Ejemplo: +54-9115593-6569")
tel = input("INTRODUZCA EL NUMERO AQUI: ")

print(tel[4:11])
    # variable[1:2] variable con corchetes y 2 valores adentro
    # desde que posicion hasta que posicion quiero imprimir.